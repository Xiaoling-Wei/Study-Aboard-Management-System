<template>
  <div>
    <div style="margin: 10px 0">
      <el-select style="width: 200px;margin: 10px;" v-model="query.country" filterable placeholder="请选择国家">
        <el-option v-for="country in countryList" :label="country.Chinese" :value="country.Chinese"
          :key="country.id"></el-option>
      </el-select>
      <el-select style="width: 200px;margin: 10px;" v-model="query.language" filterable placeholder="请选择语言类型">
        <el-option v-for="(language, index) in languageList" :label="language" :value="language" :key="index"></el-option>
      </el-select>
      <el-input style="width: 200px;margin: 10px;" placeholder="请输入GPA" suffix-icon="el-icon-search"
        v-model="query.gpa"></el-input>
      <el-input style="width: 200px;margin: 10px;" placeholder="请输入GRE" suffix-icon="el-icon-search"
        v-model="query.gre"></el-input>
      <el-input style="width: 200px;margin: 10px;" placeholder="请输入GMAT" suffix-icon="el-icon-search"
    ...
    <div v-if="tableData.length > 0">
      <h4 style="text-align: center;">
        {{ "匹配成功：" + tableData[0].name }}
      </h4>
      <div>
        <el-button type="primary">案例上传</el-button><el-button type="primary"
          @click="$router.push('/file')">学习资料</el-button><el-button type="primary">讨论区</el-button><el-button
          type="primary">院校介绍</el-button>
      </div>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    load() {
      this.request.get("/colleges/page", {
        params: {
    ...
    changeEnable(row) {
      this.request.post("/colleges/update", row).then(res => {
        if (res.code === '200') {
    ...
    del(id) {
      this.request.delete("/colleges/" + id).then(res => {
        if (res.code === '200') {
    ...
    delBatch() {
      ...
      this.request.post("/colleges/del/batch", ids).then(res => {
        if (res.code === '200') {
    ...
    save() {
      this.request.post("/colleges", this.form).then(res => {
        if (res.code === '200') {
