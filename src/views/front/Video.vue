<template>
  <div style="padding: 10px">
    <div class="requrie">
      <el-select style="width: 400px;margin: 10px;" v-model="query.country" filterable placeholder="请选择国家">
        <el-option v-for="country in countryList" :label="country.Chinese" :value="country.Chinese"
          :key="country.id"></el-option>
      </el-select>
    </div>
    <div class="requrie">
      <el-input style="width: 400px;margin: 10px;" placeholder="请输入学校昵称" suffix-icon="el-icon-search"
        v-model="query.name"></el-input>
    </div>
      <div class="requrie">
      <el-input style="width: 400px;margin: 10px;" placeholder="请输入GPA" suffix-icon="el-icon-search"
        v-model="query.gpa"></el-input>
    </div>
    <div >
      <el-select style="width: 400px;margin: 10px;" v-model="query.language" filterable placeholder="请选择语言类型">
        <el-option v-for="(language, index) in languageList" :label="language" :value="language" :key="index"></el-option>
      </el-select>
    </div>
    <div >
      <el-input style="width: 400px;margin: 10px;" placeholder="请输入语言成绩" suffix-icon="el-icon-search"
        v-model="query.score"></el-input>
    </div>
  
    <div >
      <el-input style="width: 400px;margin: 10px;" placeholder="请输入GRE" suffix-icon="el-icon-search"
        v-model="query.gre"></el-input>
    </div>
    <div>
      <el-input style="width: 400px;margin: 10px;" placeholder="请输入GMAT" suffix-icon="el-icon-search"
...
export default {
  ...
  methods: {
    save() {
      ...
      if(!this.query.name || !this.query.gpa || !this.query.country){
          return  this.$message.error("必须输入学院昵称、地区、GPA")
      }

      this.request.post("/colleges", this.query).then(res => {
        if (res.code === '200') {
