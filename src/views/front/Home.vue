<template>
  <div class="home-box">
    <div class="huati">
      <h4 style="height: 30px;border-bottom: 1px solid #f2f2f2;">热门话题</h4>
      <ul>
        <li class="huati-item" v-for="art in tableData" :key="art.id"
          @click="$router.push('/front/articleDetail?id=' + art.id)">- {{ art.name }}</li>
      </ul>
    </div>
    <div class="pipei">
      <div style="display: flex;flex-wrap: wrap;">
        <el-select style="width: 130px;margin: 10px;" v-model="query.country" filterable placeholder="请选择国家">
          <el-option v-for="country in countryList" :label="country.Chinese" :value="country.Chinese"
            :key="country.id"></el-option>
        </el-select>
        <el-select style="width: 130px;margin: 10px;" v-model="query.language" filterable placeholder="请选择语言类型">
          <el-option v-for="(language, index) in languageList" :label="language" :value="language"
            :key="index"></el-option>
        </el-select>
        <el-input style="width: 130px;margin: 10px;" placeholder="请输入GPA" suffix-icon="el-icon-search"
          v-model="query.gpa"></el-input>
        <el-input style="width: 130px;margin: 10px;" placeholder="请输入GRE" suffix-icon="el-icon-search"
          v-model="query.gre"></el-input>
        <el-input style="width: 130px;margin: 10px;" placeholder="请输入GMAT" suffix-icon="el-icon-search"
          v-model="query.gmat"></el-input>
           <el-input style="width: 130px;margin: 10px;" placeholder="请输入语言成绩" suffix-icon="el-icon-search"
          v-model="query.score"></el-input>
        <div style="width: 320px;height: 60px;line-height: 60px;text-align: center;">
          <el-button type="primary" @click="loadColleges">开始匹配</el-button>
        </div>
        <div style="width: 320px;height: 120px;text-align: center;">
          <span v-if="colleges.length >= 1"> 匹配结果：{{ colleges }} </span><span v-else>暂无匹配院校 </span>
        </div>
      </div>
...
export default {
  ...
  data() {
    return {
      ...
      countryList: [{ "English": "Singapore", "Chinese": "新加坡", "id": "1" },
  ...
  methods: {
    ...
    loadColleges() {
      this.request.get("/colleges/findBy", {
        params: {
          ...this.query
        }
      }).then(res => {
